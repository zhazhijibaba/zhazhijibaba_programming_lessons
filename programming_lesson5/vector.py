...
# a funnel surface, z = c1*ln(x**2 + y**2)
# the bottom of z coordinate, z0 = 0.0
# the radius of the bottom circle r1=1.0
# wall thickness r2=0.1
# number of point each layer n=6
# number of layers m = 6,
def make_funnel_mesh(mesh, p0, p1, p2=[1.0, 0.0, 0.0], z0=0.0, c1=1.0, r1=1.0, r2=0.1, n=3, m=3):
    p0 = np.array(p0)
    p1 = np.array(p1)
    v1 = np.array([p1[0] - p0[0], p1[1] - p0[1], p1[2] - p0[2]])
    v2 = np.array([p2[0] - p1[0], p2[1] - p1[1], p2[2] - p1[2]])
    ...
    if np.linalg.norm(v3) == 0:
        return
    v3_u0 = v3 / np.linalg.norm(v3)
    delta = 2.0 * np.pi / n
    rmat = rotation_matrix(v1, delta)

    # make points for each layer
    aaa1 = []
    aaa2 = []
    vz = v1 / float(m)
    for i in range(m):
        pz = p0 + i * vz
        z = np.linalg.norm(i * vz)
        # a funnel surface, z = r1*ln(x**2 + y**2)
        dz = -z0
        rr1 = np.sqrt(np.exp((z + z0)/ c1))
        if i == 0:
            ratio = r1 / rr1
        rr2 = rr1 + r2
        print (z + dz) * ratio,  rr2 * ratio
        aa1 = []
        aa2 = []
        v3_u = v3_u0
        for i in range(n):
            v3_u = np.dot(rmat, np.array(v3_u).T).T
            aa1.append(mesh.add_vertex((pz + v3_u * rr1 + np.array([0, 0, dz]))*ratio))
            aa2.append(mesh.add_vertex((pz + v3_u * rr2 + np.array([0, 0, dz]))*ratio))
        aaa1.append(aa1)
        aaa2.append(aa2)
    aaa2.reverse()
    ww = aaa1 + aaa2
